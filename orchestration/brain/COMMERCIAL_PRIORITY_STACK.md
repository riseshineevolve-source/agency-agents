...
|---|---|---|---|
| **#1** | **Happy Makers Detective Academy -> KDP** | Closest path to real revenue and does not depend on DUNS or Google Play. | Push English Book 1 through ALL-15 validation -> canonical integration -> final interior -> KDP preflight. Owner retains final KDP publish, pricing, cover and physical-proof gates. |
| **#2** | **Mind Bloom -> Private DONE** | Close the investment cleanly, freeze spend, preserve the architecture for a possible future commercial fork. | **Private V1 Source Release Candidate = PASS / CI #81 SUCCESS / feature development frozen.** No further Codex/Actions unless a reproducible blocker appears or owner explicitly reopens. Commercial fork is a separate future project and must not inherit private owner data/configuration. |
| **#3** | **24 Gentle Steps to Christmas** | Seasonal window is becoming time-sensitive for Q4/Christmas. | Advance source QA, Polish localization, layout-fit and publication/app preparation below owner gates. Do not sacrifice Detective #1 execution. |
| **#4** | **Optical Animals** | Strong giftable KDP product if the remaining 8 final visual slots are resolved. | Preserve 12 approved visuals. Advance tooling/manifest/preflight automatically; final 20 visual selection remains owner-gated. |
...
Detective PL is handled through the Polish Localization Engine and starts full-book production only after the English canonical master is frozen and ALL-15 spatial validation passes.
