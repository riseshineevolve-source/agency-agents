...
case "$MODE" in
  ...
  smart|full|all)
    INSTALL_ARGS=()
    VERIFY_MODE="smart"
    LABEL="full Agency library with RSE smart routing"
    ;;
  *)
...
# Build a searchable routing catalog next to the Codex configuration.
...
if [[ "$MODE" == "core" ]]; then
  echo "Installed CORE only. For the full 280-agent library, run: bash rse/install-codex.sh smart"
elif [[ "$MODE" == "curated" ]]; then
  echo "Installed the curated RSE roster. For the full library, run: bash rse/install-codex.sh smart"
else
  echo "Full Agency library is available to Codex; RSE Orchestrator should route CORE-first and use specialists on demand."
fi
