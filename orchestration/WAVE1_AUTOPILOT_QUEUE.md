...
| C1 | Detective Academy | AUTO + VERIFY through integration / preflight | **ALL-15 = 15/15 PASS locally**; 15 hybrid PDFs + 30 relabelled assets validated. Hardening commit `d5e1507` is local/push-pending because remote advanced to `02df9c2`. Preserve local work, reconcile non-destructively, push, then attach canonical assets, generate final interior and run KDP preflight. PyYAML runtime configuration is a technical blocker only; use the existing renderer dependency environment rather than reopening product scope. | KDP upload/publication, pricing, final cover and physical proof remain owner-only gates. |
