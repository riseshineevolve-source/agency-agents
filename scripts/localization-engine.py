...
from localization.gates import qa, markdown_report
from localization.io import ContractError, load, dump, leaves
...
def main(argv=None):
    ...
    try:
        if args.command == "terms-doc":
            ...
            dump(args.output, {"source_file": args.source, "paths": [{"source_path": p, "type": type(v).__name__, "value": v} for p, v in leaves(load(args.source))]})
            ...
            manifest, targets = extract(load(args.source), plan, args.source, args.revision)
            ...
            if args.command == "export":
                dump(args.output, package(manifest, targets, terms, phrases(), args.release))
